...
const app = express();
const PORT = 3859;
const HTML_FILE = path.join(__dirname, 'substack-outreach.html');
const DATA_DIR = path.join(__dirname, 'data');
const DATA_FILE = path.join(DATA_DIR, 'substack-prospects.json');
const ACTIVITY_FILE = path.join(DATA_DIR, 'substack-activity.json');
...
function backupData() {
  ...
  try {
    ...
    const backupFile = path.join(BACKUP_DIR, `substack-prospects_${timestamp}.json`);
    fs.copyFileSync(DATA_FILE, backupFile);
    // Prune old backups — keep last MAX_BACKUPS
    const backups = fs.readdirSync(BACKUP_DIR)
      .filter(f => f.startsWith('substack-prospects_'))
      .sort()
      .reverse();
